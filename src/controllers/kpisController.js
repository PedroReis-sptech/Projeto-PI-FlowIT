...
function buscarMetaDiaria(req, res) {

  kpisModel.buscarMetaDiaria().then((resultado) => {
    if (resultado.length > 0) {
      res.status(200).json(resultado);
    } else {
      res.status(204).json([]);
...
function buscarSomaDosRegistrosDosSetoresNoDia(req, res) {

  kpisModel.buscarSomaDosRegistrosDosSetoresNoDia().then((resultado) => {
    if (resultado.length > 0) {
      res.status(200).json(resultado);
    } else {
      res.status(204).json([]);
